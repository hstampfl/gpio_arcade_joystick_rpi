...
// Map of the gpios :                     up, down, left, right, start, select, a,  b,  tr, y,  x,  tl,  hotkey 
static const int mk_arcade_gpio_maps[] = {4,  17,    27,  22,    10,    9,      25, 24, 23, 18, 15, 14,  2 };
// 2nd joystick on the b+ GPIOS                 up, down, left, right, start, select, a,  b,  tr, y,  x,  tl, hotkey 
...
static const char *mk_names[] = {
    NULL, "GPIO Controller 1 13Btn", "GPIO Controller 2 13Btn"
};
...
static int __init mk_setup_pad(struct mk *mk, int idx, int pad_type_arg) {
    ...
    else (pad_type_arg == MK_ARCADE_GPIO_BPLUS) {
